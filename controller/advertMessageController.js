...
exports.getVoucherPercentage = catchAsync(async (req, res, next) => {
  const rate = (
    await db.query("SELECT percentage_rate FROM voucher_interest WHERE id = 1")
  )[0][0];
  if (!rate)
    return next(
      new AppError("No interest rate set yet, please set a new one", 400)
    );
  res.status(200).json({
    status: "success",
...
exports.changeVoucherPercentage = catchAsync(async (req, res, next) => {
  const { rate } = req.body;
  if (!Number(rate))
    return next(new AppError("Percentage rate must be a number", 400));
  if (!rate) return next(new AppError("No rate is defined", 400));

  // =============== FINDING IF RATE EXISTS BEFORE ==================== //
  const existingRate = (
    await db.query("SELECT * FROM voucher_interest WHERE id = 1")
  )[0][0];
  if (existingRate) {
    const updateRate = await db.query(
      `UPDATE voucher_interest SET percentage_rate = ${rate} WHERE id = 1`
    );
  } else {
    const newRate = await db.query(
      "INSERT INTO voucher_interest SET percentage_rate = ?",
      rate
    );
  }
  res.status(200).json({
